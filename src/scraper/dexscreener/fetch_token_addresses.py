...
def _get_driver() -> WebDriver:
    """
    Инициализирует **локальный** экземпляр Chrome/Chromium.
    Скрипт теперь открывает браузер *на вашей машине*,
    а не подключается к удалённому Selenium Standalone.

    Требования:
    • Установлен браузер Chrome (или Chromium) на ПК;
    • Установлен совместимый chromedriver (должен быть в $PATH
      или указан через переменную окружения CHROMEDRIVER_BINARY).
    """
    opts = webdriver.ChromeOptions()
    opts.add_argument("--no-sandbox")
    opts.add_argument("--disable-dev-shm-usage")
    ...
    # Если нужен headless‑режим (без графического окна)
    # раскомментируйте строку ниже:
    # opts.add_argument("--headless=new")

    # Путь до chromedriver, если он не в $PATH
...
def run(max_age_hours: int, out_dir: Path) -> str:
    ...
    try:
        driver.get(url)
        print("[wait] Solve CAPTCHA in the opened browser; waiting for table…")
        table_el = _wait_table(driver)
