#!/usr/bin/env python3
"""
sync_scraper.py  —  batch-scraper gmgn.ai c поддержкой «личностей» на IP.

✦ Для каждого прокси-IP создаётся 3 фиксированных browser-fingerprints,
  которые ротируются раз в 200 запросов или 30 минут.
✦ Сами прокси продолжают меняться циклически, как раньше.
"""

from __future__ import annotations

import asyncio, json, os, random, threading, time, uuid
...
# счётчик запросов на каждый IP для триггера ротации личности
...
def rotate_proxy(worker_id: int) -> None:
    """Round-robin прокси-IP для конкретного воркера."""
    with PROXY_LOCK:
        if not PROXY_POOL:
            return
...
def fetch_wallet_stat(worker_id: int, wallet: str) -> Optional[dict]:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        # --- берём fingerprint для текущего IP ---
        ...
        try:
            resp = curl.get(
                ...
                timeout=API_TIMEOUT, proxies=proxies,
            )
...
async def worker_chunk(
    ...
    wallets: List[str],
    *,
    token: str, total: int, processed: List[int],
) -> None:
...
# ────────────────────────── batch handler ─────────────────────────────
async def handle_batch(wallets: List[str], proxies: List[str], *, token: str, src: str):
    ...
    initial      = proxies[:n]     # первые n IP → воркерам
    PROXY_POOL   = proxies[n:]     # остальные в кольцевой пул
