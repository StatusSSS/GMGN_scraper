...
class ProxyManager:
    ...
    def _recycle(self) -> None:
        ...
        while self.cooling and self.cooling[0][1] <= now:
            ...
            if ip not in self.in_use:          # воркер мог взять IP раньше срока
                self.ready.append(ip)
                moved += 1
        if moved:
            logger.debug("[PM] recycled {} IP → ready={}", moved, len(self.ready))
            ...
            with self.lock:
                ...
                while self.ready:
                    ...
                    if ip not in self.in_use:
                        self.in_use.add(ip)
                        logger.debug("[PM] acquire → {} | in_use={} ready={} cooling={}",
                                     ip, len(self.in_use), len(self.ready), len(self.cooling))
                        return ip

                # ничего готового — ждём, пока «созреет» ближайший
    ...
    def release(self, ip: str) -> None:
        ...
        with self.lock:
            if ip not in self.in_use:
                logger.warning("[PM] release({}) not in use — пропускаю", ip)
                return
            self.in_use.remove(ip)

            # не кладём дубликат в cooling
            if any(ip == item[0] for item in self.cooling):
                logger.debug("[PM] {} уже в cooling", ip)
            else:
                self.cooling.append((ip, time.time() + self.cool_down_sec))
                logger.debug("[PM] release → {} | will be ready in {:.0f}s",
                             ip, self.cool_down_sec)
