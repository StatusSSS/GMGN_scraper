...
`Chrome/138.0.0.0` — совпадает с curl_cffi `impersonate="chrome138"`.
"""
...
def _build_static_headers(rnd: random.Random) -> Dict[str, str]:
    ...
    major = CHROME_MAJOR  # фиксированный major
...
def _build_static_params(rnd: random.Random) -> Dict[str, str]:
    ...
    return {
        ...
        # табличные фильтры (нужны только для некоторых API, удаляются при /wallet_stat)
        "limit": "50",
...
class Identity:
    ...
    def fresh(self) -> Tuple[Dict[str, str], Dict[str, str]]:
        """Возвращает (headers, params) с обновлёнными volatile‑частями."""
...
def init_proxies(proxy_list: List[str] | None = None) -> None:
    ...
    PROXY_IDENTITIES = {p: Identity(random.Random(f"{p}-0")) for p in proxy_list}
